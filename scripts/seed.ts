...
async function seed() {
  ...
  const [medicalCompany1] = await database
    ...
    .values({
      ...
      description: "Service de santé au travail interentreprises couvrant toute l'Île-de-France. Spécialisé dans l'accompagnement des TPE et PME.",
  ...
  const [medicalCompany2] = await database
    ...
    .values({
      ...
      description: "Centre de médecine du travail moderne et innovant. Équipe pluridisciplinaire à votre service.",
  ...
  console.log(`   ✓ Created: ${clientCompany1.name} (member of ${medicalCompany1.name})`);
  console.log(`   ✓ Created: ${clientCompany2.name} (member of ${medicalCompany2.name})`);
  ...
  await database.insert(schema.membershipRequestsTable).values({
    ...
    medicalCompanyId: medicalCompany1.id,
    status: "pending",
    message: "Bonjour, nous sommes une entreprise de construction de 45 employés. Nous recherchons un service de santé au travail fiable pour nos équipes travaillant sur différents chantiers en Île-de-France.",
  });

  console.log(`   ✓ Created pending request: ${clientCompany3.name} → ${medicalCompany1.name}`);
  ...
  console.log("   ✓ Created 4 bookings (2 upcoming, 1 completed, 1 for boulangerie)");
